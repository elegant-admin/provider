...
if (!function_exists('admin_trans')) {
    ...
    {
        $line = __('admin.' . $key, $replace, $locale);

        if (!is_string($line) || substr($line, 0, 5) === 'admin') {
            return $key;
        }
    ...
    function get_routes()
    {
        ...
        foreach (app('router')->getRoutes() as $route) {
            ...
            if (isset($route->action['as'])) {
                if (!in_array($uri, config('admin.auth.excepts')) && mb_strpos($route->action['as'], 'admin') !== false) {
                    $uri = set_route_url($uri);
